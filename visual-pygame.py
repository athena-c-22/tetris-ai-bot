...
# def generate_genome():
#     difference = uniform(-5, 5)
#     genome = [-19.916366988549214 + difference, -0.27026029511791627 + difference, -1.5392396093619634 + difference, -0.7875854176515529 + difference, 3.7747020546884453 + difference, -100.76555287796035 + difference, 0.42446341412810096 + difference]
#     return genome
...
# def mutate(genome, num=2, probability=0.5):
#     for _ in range(num):
#         index = randrange(len(genome))
#         genome[index] = genome[index] if random() > probability else genome[index] + uniform(-100, 100)
#     return genome

# def run_evolution(population_size, fitness_limit, generation_limit):
#     population = generate_population(population_size)

#     for i in range(generation_limit):
#         print("Generation ", i + 1)
#         population = sorted(population, key=lambda genome:fitness(genome), reverse=True)
#         fitnesses = sorted([fitness(genome) for genome in population], reverse=True)

#         if fitnesses[0] >= fitness_limit:
#             break
...
# population, generations = run_evolution(10, 20000, 3)
# print(population, generations)

if __name__ == '__main__':
    weights = [-19.916366988549214, -0.27026029511791627, -1.5392396093619634, -0.7875854176515529, 3.7747020546884453, -100.76555287796035, 0.42446341412810096]
    weights2 = [-20.110954195982956, -0.46484750255165963, -1.7338268167957067, -0.9821726250852962, 3.580114847254702, -100.96014008539409, 0.2298762066943576]
    run()
